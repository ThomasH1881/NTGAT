...
from dgl.ops import edge_softmax, my_edge_softmax
    ...
    def forward(self, graph, feat):
        with graph.local_scope():
            ...
            if self.training and self.edge_drop > 0:
                ...
                eids = perm[bound:]
                graph.edata["a"] = torch.zeros_like(e)
                graph.edata["a"][eids] = self.attn_drop(my_edge_softmax(graph, e[eids], self.lowerbound, self.trunc_k, eids=eids))
                #graph.edata["a"][eids] = self.attn_drop(my_edge_softmax(graph, e[eids]))
            else:
            ...
            for i in range(graph.num_nodes()):
                N = len(graph.in_edges(i, form='eid'))
                if (N > 50):
                    x = random.randint(1,100)
                    if (x<5):
                      index = graph.in_edges(i, form='eid').type(torch.int64)
                      tensor = graph.edata['a'][:,0,:][index].squeeze()
                      tesnor_s = graph.edata['a'][:,0,:][index].squeeze().argsort()
  
                      #mask = tesnor_s[math.ceil(0.99*N) : N]
                      #print("Largest 1%:", tensor[mask].sum(dim=0))
                      #mask = tesnor_s[math.ceil(0.95*N) : N]
                      #print("Largest 5%:", tensor[mask].sum(dim=0))
                      #mask = tesnor_s[math.ceil(0.9*N) : N]
                      #print("Largest 10%:", tensor[mask].sum(dim=0))
                      #mask = tesnor_s[math.ceil(0.8*N) : N]
                      #print("Largest 20%:", tensor[mask].sum(dim=0))
                      mask = tesnor_s[math.ceil(0.5*N) : N]
                      print("Largest 50%:", tensor[mask].sum(dim=0))
                      k = k + 1
                if k == 100:
                    break
            '''
            ...
            if (i == n_layers - 1):
                ...
                self.convs.append(
                    my_GATConv(
                        ...
                        use_symmetric_norm=use_symmetric_norm,
                        residual=True,
                        lowerbound=self.lowerbound*(i+1), 
                        trunc_k=self.trunc_k*(0.9**i)
                    )
                )
