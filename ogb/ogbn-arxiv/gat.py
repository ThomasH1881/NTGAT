...
def load_data(dataset):
    ...
    data = DglNodePropPredDataset(name=dataset, root='/home/nfs_data/datasets/')
    ...
    with torch.profiler.profile(
        activities = [
            ...
            for iter in range(50):
                pred = model(graph, feat)
                p.step()
    p.export_chrome_trace('arxiv.json')
    print(p.key_averages().table(sort_by='self_cuda_time_total', row_limit=-1))
    '''
    
    '''
    for i in range(20):
        pred = model(graph, tmp)
...
def main():
    ...
    argparser.add_argument("--lb", type=int, default=100,
                        help="lowerbound threshold")
    argparser.add_argument("--k", type=float, default=0.9,
                        help="trunck rate k")
